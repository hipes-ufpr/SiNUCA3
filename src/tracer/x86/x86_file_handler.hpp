...
// Not using enum since it may vary in size depending on machine
...
/**
 * @brief DataINS is a packed struct since it is written directly to the static
 * trace (binary file). It is designed to take as little space as possible while
...
/**
 * @brief DataMEM is a packed struct since it is written directly to the memory
 * trace (binary file). It is designed to take as little space as possible while
 * storing memory access data.
 */
struct DataMEM {
    unsigned long addr;
    unsigned int size;
